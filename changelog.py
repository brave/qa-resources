	...
	for issue in ios_repo.get_issues(milestone=ios_milestone[ios_key],  sort="created", direction="asc", state="closed"):
		if 'pull' not in issue.html_url:
			...
			if "crash" in label_names and "tests" not in label_names:
				crash_line = " - " + issue_title + ".(#" + str(issue.number) + ")"
				crash_list.append(crash_line)
			if "crash" not in label_names and "webcompat" not in label_names and "regression" not in label_names and "feature/private-tabs" not in label_names and "feature/sync" not in label_names and "feature/rewards" not in label_names and "feature/shields" not in label_names and "tests" not in label_names:
				general_line = " - " + issue_title + ".(#" + str(issue.number) + ")"
				general_list.append(general_line)
			...
			if "iPad" in label_names or "iPad Specific" in label_names:
				iPad_line = " - " + issue_title + ".(#" + str(issue.number) + ")"
				iPad_list.append(iPad_line)
			if "iPhone" in label_names or "iPhone X Specific" in label_names or "iPhone Specific" in label_names and "tests" not in label_names:
				iPhone_line = " - " + issue_title + ".(#" + str(issue.number) + ")"
				iPhone_list.append(iPhone_line)
			if "webcompat" in label_names or "audio" in label_names or "feature/video" in label_names and "tests" not in label_names:
				webcompat_line = " - " + issue_title + ".(#" + str(issue.number) + ")"
				webcompat_list.append(webcompat_line)
			if "feature/sync" in label_names:
				sync_line = " - " + issue_title + ".(#" + str(issue.number) + ")"
